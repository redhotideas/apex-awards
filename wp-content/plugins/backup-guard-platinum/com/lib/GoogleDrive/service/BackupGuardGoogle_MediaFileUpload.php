 ...
 *
 */
class BackupGuardGoogle_MediaFileUpload {
  ...
  public static function getUploadType($meta, &$payload, &$params) {
    if (isset($params['mediaUpload'])
        && get_class($params['mediaUpload']['value']) == 'BackupGuardGoogle_MediaFileUpload') {
  ...
  public function nextChunk(BackupGuardGoogle_HttpRequest $req, $chunk=false) {
    ...
    $httpRequest = new BackupGuardGoogle_HttpRequest($this->resumeUri, 'PUT', $headers, $chunk);
    $response = BackupGuardGoogle_Client::$io->authenticatedRequest($httpRequest);
    ...
    if (308 == $code) {
      ...
      return BackupGuardGoogle_REST::decodeHttpResponse($response);
    }
  }

  private function getResumeUri(BackupGuardGoogle_HttpRequest $httpRequest) {
    $result = null;
    $body = $httpRequest->getPostBody();
    if ($body) {
      $httpRequest->setRequestHeaders(array(
        'content-type' => 'application/json; charset=UTF-8',
        'content-length' => BackupGuardGoogle_Utils::getStrLen($body),
    ...
    $response = BackupGuardGoogle_Client::$io->makeRequest($httpRequest);
    $location = $response->getResponseHeader('location');
    $code = $response->getResponseHttpCode();
    if (200 == $code && true == $location) {
      return $location;
    }
    throw new BackupGuardGoogle_Exception("Failed to start the resumable upload");
  }
}
