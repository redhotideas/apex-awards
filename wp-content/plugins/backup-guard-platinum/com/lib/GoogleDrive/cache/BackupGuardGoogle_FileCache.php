 ...
 */
class BackupGuardGoogle_FileCache extends BackupGuardGoogle_Cache {
  private $path;
  ...
  private function createLock($storageFile) {
    ...
    if (! is_dir($storageDir)) {
      ...
      if (! @mkdir($storageDir, 0755, true)) {
        ...
        if (! is_dir($storageDir)) {
          throw new BackupGuardGoogle_CacheException("Could not create storage directory: $storageDir");
        }
      }
  ...
  public function set($key, $value) {
    ...
    if (! @file_put_contents($storageFile, $data)) {
      $this->removeLock($storageFile);
      throw new BackupGuardGoogle_CacheException("Could not store data in the file");
    }
    $this->removeLock($storageFile);
  }
  ...
  public function delete($key) {
    ...
    if (! @unlink($file)) {
      throw new BackupGuardGoogle_CacheException("Cache file could not be deleted");
    }
  }
}
