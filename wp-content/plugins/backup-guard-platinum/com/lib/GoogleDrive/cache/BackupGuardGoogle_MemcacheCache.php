 ...
 */
class BackupGuardGoogle_MemcacheCache extends BackupGuardGoogle_Cache {
  private $connection = false;

  public function __construct() {
    global $apiConfig;
    if (! function_exists('memcache_connect')) {
      throw new BackupGuardGoogle_CacheException("Memcache functions not available");
    }
    $this->host = $apiConfig['ioMemCacheCache_host'];
    $this->port = $apiConfig['ioMemCacheCache_port'];
    if (empty($this->host) || empty($this->port)) {
      throw new BackupGuardGoogle_CacheException("You need to supply a valid memcache host and port");
    }
  }
  ...
  private function connect() {
    if (! $this->connection = @memcache_pconnect($this->host, $this->port)) {
      throw new BackupGuardGoogle_CacheException("Couldn't connect to memcache server");
    }
  }
   ...
   * @param string $key
   * @param string $value
   * @throws BackupGuardGoogle_CacheException
   */
  public function set($key, $value) {
    ...
    if (@memcache_set($this->connection, $key, array('time' => time(),
      ...
      throw new BackupGuardGoogle_CacheException("Couldn't store data in cache");
    }
  }
