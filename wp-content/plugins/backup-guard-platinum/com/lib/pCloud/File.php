...
class File {
	...
	public function download($fileId, $destination = "") {
		...
		$source = fopen($fileLink, "rb", false, stream_context_create($arrContextOptions));
	...
	public function save($uploadId, $name, $folderId) {
		$params = array(
			"uploadid" => $uploadId,
			"name" => $name,
			"folderid" => $folderId
	...
	public function chunk_write($params, $content) {
		return $this->write($params, $content);
	}
}