...
namespace BackupGuard\Aws\Common\Client;

use BackupGuard\Aws\Common\Credentials\Credentials;
use BackupGuard\Aws\Common\Credentials\CredentialsInterface;
use BackupGuard\Aws\Common\Credentials\NullCredentials;
use BackupGuard\Aws\Common\Enum\ClientOptions as Options;
use BackupGuard\Aws\Common\Exception\ExceptionListener;
use BackupGuard\Aws\Common\Exception\InvalidArgumentException;
use BackupGuard\Aws\Common\Exception\NamespaceExceptionFactory;
use BackupGuard\Aws\Common\Exception\Parser\DefaultXmlExceptionParser;
use BackupGuard\Aws\Common\Exception\Parser\ExceptionParserInterface;
use BackupGuard\Aws\Common\Iterator\AwsResourceIteratorFactory;
use BackupGuard\Aws\Common\RulesEndpointProvider;
use BackupGuard\Aws\Common\Signature\EndpointSignatureInterface;
use BackupGuard\Aws\Common\Signature\SignatureInterface;
use BackupGuard\Aws\Common\Signature\SignatureV2;
use BackupGuard\Aws\Common\Signature\SignatureV3Https;
use BackupGuard\Aws\Common\Signature\SignatureV4;
use BackupGuard\Guzzle\Common\Collection;
use BackupGuard\Guzzle\Plugin\Backoff\BackoffPlugin;
use BackupGuard\Guzzle\Plugin\Backoff\CurlBackoffStrategy;
use BackupGuard\Guzzle\Plugin\Backoff\ExponentialBackoffStrategy;
use BackupGuard\Guzzle\Plugin\Backoff\HttpBackoffStrategy;
use BackupGuard\Guzzle\Plugin\Backoff\TruncatedBackoffStrategy;
use BackupGuard\Guzzle\Service\Description\ServiceDescription;
use BackupGuard\Guzzle\Service\Resource\ResourceIteratorClassFactory;
use BackupGuard\Guzzle\Log\LogAdapterInterface;
use BackupGuard\Guzzle\Log\ClosureLogAdapter;
use BackupGuard\Guzzle\Plugin\Backoff\BackoffLogger;
...
class ClientBuilder
{
    ...
    public function __construct($namespace = null)
    {
        ...
        $this->clientClass = 'BackupGuard\Aws\Common\Client\DefaultClient';
            ...
            if ($logger === 'debug') {
                ...
                throw new InvalidArgumentException(
                    Options::BACKOFF_LOGGER . ' must be set to `debug` or an instance of '
                        . 'BackupGuard\\Guzzle\\Common\\Log\\LogAdapterInterface'
                );
            }
    ...
    protected function getSignature(ServiceDescription $description, Collection $config)
    {
        ...
        if (is_string($signature)) {
            ...
            throw new InvalidArgumentException('The provided signature is not '
                ...
                . 'BackupGuard\\Aws\\Common\\Signature\\SignatureInterface');
        }
