...
use BackupGuard\Guzzle\Http\RedirectPlugin;
...
class RequestFactory implements RequestFactoryInterface
{
    ...
    protected $requestClass = 'BackupGuard\\Guzzle\\Http\\Message\\Request';

    /** @var string Class to instantiate for requests with a body */
    protected $entityEnclosingRequestClass = 'BackupGuard\\Guzzle\\Http\\Message\\EntityEnclosingRequest';
    ...
    protected function visit_exceptions(RequestInterface $request, $value, $flags)
    {
        if ($value === false || $value === 0) {
            ...
            foreach ($dispatcher->getListeners('request.error') as $listener) {
                if (is_array($listener) && $listener[0] == 'BackupGuard\Guzzle\Http\Message\Request' && $listener[1] = 'onRequestError') {
