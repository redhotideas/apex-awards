			...
			foreach ( $data as $row ) {
				...
				foreach ( $columns as $column ) {
					...
					*/
					...
					if (is_array($primary_keys) && in_array($column, $primary_keys)) {
		...
		try {
			...
			elseif ( is_object( $data ) && !is_a( $data, '__PHP_Incomplete_Class' ) ) {
				...
				if ( is_string( $data ) ) {
					...
					// I added this [$@] to regexp to prevent http://http:// result after restore (test case we have like this url ]http:// )
